...
exports.checkSubscriptionType = (req, res, next) => {
  const available_booking = ['month', 'year', 'resident_month', 'resident_committed'];
  if(available_booking.includes(req.params.subscription_type)){
    return next();
  }
  return next(new AppError(`booking type ${req.params.subscription_type} does not exist`, 400))
}
    ...
    line_items: [
      {
        ...
        images: ['https://www.officelovin.com/wp-content/uploads/2017/10/nakedhub-coworking-11.jpg'],
...
exports.createSubscriptionCheckout = catchAsynchronousError(async (req, res, next) => {
  const user = await User.findById(req.user.id)
})

exports.getSubscriptionPlan = (req, res, next) => next();
